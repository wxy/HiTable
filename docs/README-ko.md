...
HiTable은 4개 그룹으로 나뉜 15가지 통계 알고리즘을 지원합니다:

- **기본 통계**: CNT (개수), SUM (합계), AVG (평균), MIN (최소값), MAX (최대값), RNG (범위)
- **고급 통계**: MED (중앙값), STD (표준편차), VAR (분산), MOD (최빈값)
- **분위수 통계**: Q1 (제1사분위수), Q3 (제3사분위수), IQR (사분위범위)
- **분포 통계**: SKW (왜도), KUR (첨도)

설정 페이지에서 원하는 알고리즘을 활성화하거나 비활성화할 수 있습니다.
