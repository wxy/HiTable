...
### Поддерживаемые статистические алгоритмы

HiTable поддерживает 15 статистических алгоритмов, разделенных на 4 группы:

- **Базовая статистика**: CNT (количество), SUM (сумма), AVG (среднее), MIN (минимум), MAX (максимум), RNG (размах)
- **Расширенная статистика**: MED (медиана), STD (стандартное отклонение), VAR (дисперсия), MOD (мода)
- **Квантильная статистика**: Q1 (первый квартиль), Q3 (третий квартиль), IQR (межквартильный размах)
- **Статистика распределения**: SKW (асимметрия), KUR (эксцесс)

Вы можете включить или отключить любой алгоритм на странице настроек.
