...
HiTable 支援 15 種統計算法，分為 4 組：

- **基礎統計**：CNT（計數）、SUM（求和）、AVG（平均值）、MIN（最小值）、MAX（最大值）、RNG（全距）
- **進階統計**：MED（中位數）、STD（標準差）、VAR（方差）、MOD（眾數）
- **分位數統計**：Q1（第一四分位數）、Q3（第三四分位數）、IQR（四分位距）
- **分佈統計**：SKW（偏度）、KUR（峰度）
...
一旦啟動，你可以透過按低滑鼠左鍵並拖曳喺表格中選擇一個矩形區域。放開滑鼠以結束選擇，呢個時候將喺選擇區域外彈出一個浮層，呢個浮層喺四個邊上分別對選擇區嘅行同列進行各種統計。呢四個邊嘅逆時針左側顯示咗各個邊正在使用嘅統計算法。
